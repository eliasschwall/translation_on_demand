---
title: "Translation on Demand"
format: 
  dashboard:
    scrolling: true
logo: "CECAD-Logo-24-white.png"
theme: minty
embed-resources: true
nav-buttons:
      - icon: github
        href: https://github.com/eliasschwall/translation_on_demand
---

```{r set up}
#| label: set up chunk

library(tidyverse)
library(here)
library(DT)
library(gt)
```

# Data Overview 

## Row 
```{r update valuebox}
#| content: valuebox
#| title: "Latest update"
list(
  icon = "clock",
  color = "secondary",
  value = Sys.Date()
)
```

::: {.card title="Data Source"}
The data used for this project is from Yang et al., 2019: [Multi-omic Profiling Reveals Dynamics of the Phased Progression of Pluripotency](https://www.sciencedirect.com/science/article/pii/S2405471219301152?via%3Dihub).

- [Table S2](https://ars.els-cdn.com/content/image/1-s2.0-S2405471219301152-mmc3.xlsx): Temporal Proteomic Data (0min, 30min, 1h, 6h, 12h, 24h, 36h, 48h, and 72h)
- [Table S3](https://ars.els-cdn.com/content/image/1-s2.0-S2405471219301152-mmc4.xlsx): Temporal Transcriptomic (RNA-Seq) Data (0min, 1h, 6h, 12h, 24h, 36h, 48h, and 72h)

For the purpose of our project we realigned the count data for the transcriptomic data with the M11 mouse genome using STAR. 
:::

## Row

```{r displaying source data proteome}
#| title: Proteome Data Head

proteomic_yang_data <- here::here("data/processed/yang_paper", "yang_proteomics.rds") %>% readRDS()

proteomic_yang_data %>% 
  head() %>% 
  gt()
```

## Row

```{r displaying source data transcriptome}
#| title: Transcriptome Data Head

transcriptomic_yang_data <- here::here("data/raw/yang_paper", "transcriptomic_counts.csv") |> read.csv()

transcriptomic_yang_data %>% 
  head() %>% 
  gt()
```

# ToD Filtering 

## Row {.tabset title="ToD candidate genes" height=100%}
```{r}
#| title: 0h to 1h

ToD_candidates_list <- readRDS(here::here("data/processed", "ToD_candidates_list.rds"))

datatable(ToD_candidates_list$`1h_vs_0h`, 
          extensions = 'Buttons',
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_candidates_0h_to_1h'),
              list(extend = 'excel', title = 'ToD_candidates_0h_to_1h')
            )
          )
)
```

```{r}
#| title: 1h to 6h

datatable(ToD_candidates_list$`6h_vs_1h`, 
          extensions = 'Buttons',
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_candidates_1h_to_6h'),
              list(extend = 'excel', title = 'ToD_candidates_1h_to_6h')
            )
          )
)
```

```{r}
#| title: 6h to 12h

datatable(ToD_candidates_list$`12h_vs_6h`, 
          extensions = 'Buttons',
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_candidates_6h_to_12h'),
              list(extend = 'excel', title = 'ToD_candidates_6h_to_12h')
            )
          )
)
```

```{r}
#| title: 12h to 24h

datatable(ToD_candidates_list$`24h_vs_12h`, 
          extensions = 'Buttons',
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_candidates_12h_to_24h'),
              list(extend = 'excel', title = 'ToD_candidates_12h_to_24h')
            )
          )
)
```

```{r}
#| title: 24h to 36h

datatable(ToD_candidates_list$`36h_vs_24h`, 
          extensions = 'Buttons',
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_candidates_24h_to_36h'),
              list(extend = 'excel', title = 'ToD_candidates_24h_to_36h')
            )
          )
)
```

```{r}
#| title: 36h to 48h

datatable(ToD_candidates_list$`48h_vs_36h`, 
          extensions = 'Buttons',
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_candidates_36h_to_48h'),
              list(extend = 'excel', title = 'ToD_candidates_36h_to_48h')
            )
          )
)
```

```{r}
#| title: 48h to 72h

datatable(ToD_candidates_list$`72h_vs_48h`, 
          extensions = 'Buttons',
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_candidates_48h_to_72h'),
              list(extend = 'excel', title = 'ToD_candidates_48h_to_72h')
            )
          )
)
```

# GO Enrichment 

## Row 

### Column {.tabset title="GO Enrichments"}
```{r}
#| title: 0h to 1h BP

enrichment_plots <- here::here("results/GO_Kegg_Enrichment_ToD_candidates", "GO_Kegg_Enrichment_ToD_candidates.rds") %>% readRDS()

enrichment_plots$`1h_vs_0h_BP`
```

```{r}
#| title: 0h to 1h CC

enrichment_plots$`1h_vs_0h_CC`
```

```{r}
#| title: 0h to 1h MF

enrichment_plots$`1h_vs_0h_MF`
```

```{r}
#| title: 1h to 6h BP

enrichment_plots$`6h_vs_1h_BP`
```

```{r}
#| title: 1h to 6h CC

enrichment_plots$`6h_vs_1h_CC`
```

```{r}
#| title: 1h to 6h MF

enrichment_plots$`6h_vs_1h_MF`
```

```{r}
#| title: 6h to 12h BP

enrichment_plots$`12h_vs_6h_BP`
```

```{r}
#| title: 6h to 12h CC

enrichment_plots$`12h_vs_6h_CC`
```

```{r}
#| title: 6h to 12h MF

enrichment_plots$`12h_vs_6h_MF`
```

```{r}
#| title: 12h to 24h BP

enrichment_plots$`24h_vs_12h_BP`
```

```{r}
#| title: 12h to 24h CC

enrichment_plots$`24h_vs_12h_CC`
```

```{r}
#| title: 12h to 24h MF

enrichment_plots$`24h_vs_12h_MF`
```

```{r}
#| title: 24h to 36h BP

enrichment_plots$`36h_vs_24h_BP`
```

```{r}
#| title: 24h to 36h CC

enrichment_plots$`36h_vs_24h_CC`
```

```{r}
#| title: 24h to 36h MF

enrichment_plots$`36h_vs_24h_MF`
```

```{r}
#| title: 36h to 48h BP

enrichment_plots$`48h_vs_36h_BP`
```

```{r}
#| title: 36h to 48h CC

enrichment_plots$`48h_vs_36h_CC`
```

```{r}
#| title: 36h to 48h MF

enrichment_plots$`48h_vs_36h_MF`
```

```{r}
#| title: 48h to 72h BP

enrichment_plots$`72h_vs_48h_BP`
```

```{r}
#| title: 48h to 72h CC

enrichment_plots$`72h_vs_48h_CC`
```

```{r}
#| title: 48h to 72h MF

enrichment_plots$`72h_vs_48h_MF`
```

### Column {.tabset title="KEGG Enrichments"}

```{r}
#| title: 0h to 1h 

enrichment_plots$p_1h_vs_0h_KEGG
```

```{r}
#| title: 1h to 6h 

enrichment_plots$p_6h_vs_1h_KEGG
```

```{r}
#| title: 6h to 12h 

enrichment_plots$p_12h_vs_6h_KEGG
```

```{r}
#| title: 12h to 24h 

enrichment_plots$p_24h_vs_12h_KEGG
```

```{r}
#| title: 24h to 36h 

enrichment_plots$p_36h_vs_24h_KEGG
```

```{r}
#| title: 36h to 48h 

enrichment_plots$p_48h_vs_36h_KEGG
```

```{r}
#| title: 48h to 72h 

enrichment_plots$p_72h_vs_48h_KEGG
```

# TF Analysis

```{r}
#| title: TF Prot-RNA-LFC-Ratio Over Time
#| label: looking at the temporal behavior of ToD TFs

mouse_TFs <- here("data/reference", "Mus_musculus_TF.txt") %>% read_tsv()
matched_list <- readRDS(here::here("data/processed", "matched_list.rds"))

ToDCs_TFs <- lapply(ToD_candidates_list, function(time_interval){
  intersect(time_interval$mgi_symbol,mouse_TFs$Symbol)
}) 

# merge all time points in one df
merged_matched <- Reduce(function(x, y) merge(x, y, by = "mgi_symbol", all = TRUE), matched_list) %>%
  janitor::clean_names() %>%
  select(mgi_symbol, colnames(.)[str_detect(colnames(.), "diff")]) %>%
  setNames(c("gene", "h0_h1", "h1_h6", "h6_h12", "h12_h24", "h24_h36", "h36_h48", "h48_h72"))

# filtering this df for all ToD TFs
ToDCs_TFs_all <- c(
  ToDCs_TFs$`1h_vs_0h`,
  ToDCs_TFs$`6h_vs_1h`,
  ToDCs_TFs$`12h_vs_6h`,
  ToDCs_TFs$`24h_vs_12h`,
  ToDCs_TFs$`36h_vs_24h`,
  ToDCs_TFs$`48h_vs_36h`,
  ToDCs_TFs$`72h_vs_48h`
)

merged_matched_TFs <- merged_matched %>% filter(gene %in% ToDCs_TFs_all)

# Transform the data for ggplot
data_long <- merged_matched_TFs %>%
  pivot_longer(cols = -gene, names_to = "timepoint", values_to = "value") %>%
  mutate(timepoint = recode(timepoint,
    h0_h1 = "0-1h",
    h1_h6 = "1-6h",
    h6_h12 = "6-12h",
    h12_h24 = "12-24h",
    h24_h36 = "24-36h",
    h36_h48 = "36-48h",
    h48_h72 = "48-72h"
  )) %>% 
    mutate(timepoint = factor(timepoint, levels = c("0-1h","1-6h","6-12h","12-24h","24-36h","36-48h","48-72h")))

# Create the plot
p <- ggplot(data_long, aes(x = timepoint, y = value, group = gene, color = gene)) +
  geom_line() +
  geom_point() +
  labs(y = "Prot-RNA-LFC-Ratio", x="") +
  geom_hline(yintercept = 1, linetype = "dashed", color = "red") +
  annotate("text", x = 7.5, y = 1.5, label = "ToDC", color = "red")+
theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

# Convert to plotly
p_plotly <- plotly::ggplotly(p, tooltip = c("gene", "value"))

# Set all traces to be turned off by default
for (i in 1:length(p_plotly$x$data)) {
  if (i != 105 && i != 106) {  # Replace y_intercept_index with the actual index of your y-intercept line
    p_plotly$x$data[[i]]$visible <- "legendonly"
  }
}

# Show the plot
p_plotly
```

## Row
```{r Pathway activity}
#| include: false
library(ggrepel)
dds <- here::here("data/processed/","dds.rds") %>% readRDS()

design <- SummarizedExperiment::colData(dds) %>% as_tibble()
res_1_vs_0 <- DESeq2::results(dds, contrast = c("timepoint", "1h", "0h")) %>% 
  as.data.frame() %>% 
  dplyr::select(log2FoldChange, stat, padj) %>% 
  rownames_to_column("ensembl_id") %>% 
  mutate(ensembl_id = sub("\\..*", "", ensembl_id)) %>% 
  mutate(symbol = AnnotationDbi::mapIds(org.Mm.eg.db::org.Mm.eg.db,
                          keys = ensembl_id,
                          column = "SYMBOL",
                          keytype = "ENSEMBL",
                          multiVals = "first")) %>% 
  dplyr::select(-ensembl_id) %>% 
  group_by(symbol) %>%
  summarise(across(everything(), mean, na.rm = TRUE), .groups = 'drop') %>%
  ungroup() %>% 
  dplyr::filter(!is.na(symbol)) %>% 
  column_to_rownames("symbol") %>% 
  dplyr::select(stat)

res_6_vs_1 <- DESeq2::results(dds, contrast = c("timepoint", "6h", "1h")) %>% 
  as.data.frame() %>% 
  dplyr::select(log2FoldChange, stat, padj) %>% 
  rownames_to_column("ensembl_id") %>% 
  mutate(ensembl_id = sub("\\..*", "", ensembl_id)) %>% 
  mutate(symbol = AnnotationDbi::mapIds(org.Mm.eg.db::org.Mm.eg.db,
                          keys = ensembl_id,
                          column = "SYMBOL",
                          keytype = "ENSEMBL",
                          multiVals = "first")) %>% 
  dplyr::select(-ensembl_id) %>% 
  group_by(symbol) %>%
  summarise(across(everything(), mean, na.rm = TRUE), .groups = 'drop') %>%
  ungroup() %>% 
  dplyr::filter(!is.na(symbol)) %>% 
  column_to_rownames("symbol") %>% 
  dplyr::select(stat)

net_path <- decoupleR::get_progeny(organism = 'mouse', top = 500)
contrast_acts_1_vs_0 <- decoupleR::run_mlm(mat=res_1_vs_0, net=net_path, .source='source', .target='target', .mor='weight', minsize = 5)

contrast_acts_6_vs_1 <- decoupleR::run_mlm(mat=res_6_vs_1, net=net_path, .source='source', .target='target', .mor='weight', minsize = 5)
```

### Column
```{r} 
#| title: Core Pathways regulation between 1h and 0h               
ggplot(contrast_acts_1_vs_0, aes(x = reorder(source, score), y = score)) + 
    geom_bar(aes(fill = score), stat = "identity") +
    scale_fill_gradient2(low = "darkblue", high = "indianred", 
        mid = "whitesmoke", midpoint = 0) + 
    theme_minimal() +
    theme(axis.title = element_text(face = "bold", size = 12),
        axis.text.x = 
            element_text(angle = 45, hjust = 1, size =10, face= "bold"),
        axis.text.y = element_text(size =10, face= "bold"),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
    xlab("Pathways") 
```

### Column {.tabset title="Specific Pathway regulators between 1h and 0h"}
```{r}
#| title: MAPK

pathway <- 'MAPK'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_1_vs_0),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_1_vs_0[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted') +
  ylab("DESeq stats")
```

```{r}
#| title: PI3K

pathway <- 'PI3K'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_1_vs_0),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_1_vs_0[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

```{r}
#| title: EGFR

pathway <- 'EGFR'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_1_vs_0),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_1_vs_0[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

```{r}
#| title: TGFb

pathway <- 'TGFb'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_1_vs_0),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_1_vs_0[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

```{r}
#| title: WNT

pathway <- 'WNT'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_1_vs_0),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_1_vs_0[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

```{r}
#| title: JAK-STAT

pathway <- 'JAK-STAT'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_1_vs_0),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_1_vs_0[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

## Row

### Column
```{r} 
#| title: Core Pathways regulation between 6h and 1h               
ggplot(contrast_acts_6_vs_1, aes(x = reorder(source, score), y = score)) + 
    geom_bar(aes(fill = score), stat = "identity") +
    scale_fill_gradient2(low = "darkblue", high = "indianred", 
        mid = "whitesmoke", midpoint = 0) + 
    theme_minimal() +
    theme(axis.title = element_text(face = "bold", size = 12),
        axis.text.x = 
            element_text(angle = 45, hjust = 1, size =10, face= "bold"),
        axis.text.y = element_text(size =10, face= "bold"),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
    xlab("Pathways") 
```

### Column {.tabset title="Specific Pathway regulators between 6h and 1h"}
```{r}
#| title: MAPK

pathway <- 'MAPK'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_6_vs_1),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_6_vs_1[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted') +
  ylab("DESeq stats")
```

```{r}
#| title: PI3K

pathway <- 'PI3K'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_6_vs_1),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_6_vs_1[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

```{r}
#| title: EGFR

pathway <- 'EGFR'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_6_vs_1),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_6_vs_1[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

```{r}
#| title: TGFb

pathway <- 'TGFb'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_6_vs_1),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_6_vs_1[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

```{r}
#| title: WNT

pathway <- 'WNT'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_6_vs_1),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_6_vs_1[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

```{r}
#| title: JAK-STAT

pathway <- 'JAK-STAT'

df <- net_path %>%
  filter(source == pathway) %>%
  arrange(target) %>%
  mutate(ID = target, color = "3") %>%
  column_to_rownames('target')
inter <- sort(intersect(rownames(res_6_vs_1),rownames(df)))
df <- df[inter, ]
df['t_value'] <- res_6_vs_1[inter, ]
df <- df %>%
  mutate(color = if_else(weight > 0 & t_value > 0, '1', color)) %>%
  mutate(color = if_else(weight > 0 & t_value < 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value > 0, '2', color)) %>%
  mutate(color = if_else(weight < 0 & t_value < 0, '1', color))

ggplot(df, aes(x = weight, y = t_value, color = color)) + geom_point() +
  scale_colour_manual(values = c("red","royalblue3","grey")) +
  geom_label_repel(aes(label = ID)) + 
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = 'dotted') +
  geom_hline(yintercept = 0, linetype = 'dotted')
```

# Motif Analysis 

## Row

### Column

```{r}
#| title: RNA Binding Protein Target Motifs Enriched in ToD Candidates

# Group the data by time_interval
grouped_data <- readRDS(here::here("results/meme_analysis", "RBP_motif_enrichmets_df.rds")) %>%
  group_by(time_interval)

# Create the DT table
datatable(grouped_data, 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'RBP_Motif_Enrichments_in_ToD_candidates'),
              list(extend = 'excel', title = 'RBP_Motif_Enrichments_in_ToD_candidates')
            )
          )
)
```

### Column {.tabset title="ToD candidates carrying RBP motifs"} 
```{r}
#| title: 0h to 1h 5' UTRs

ToD_genes_carrying_RBP_motifs <- here::here("results/meme_analysis", "ToD_genes_carrying_RBP_motifs.rds") %>% 
  readRDS()

ToD_genes_carrying_RBP_motifs$`1h_vs_0h`$utr_5 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_0h_to_1h_5_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_0h_to_1h_5_utr')
            )
          )
)

```

```{r}
#| title: 0h to 1h 3' UTRs

ToD_genes_carrying_RBP_motifs$`1h_vs_0h`$utr_3 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_0h_to_1h_3_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_0h_to_1h_3_utr')
            )
          )
)
```

```{r}
#| title: 1h to 6h 5' UTRs

ToD_genes_carrying_RBP_motifs$`6h_vs_1h`$utr_5 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_1h_to_6h_5_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_1h_to_6h_5_utr')
            )
          )
)
```

```{r}
#| title: 6h to 12h 5' UTRs

ToD_genes_carrying_RBP_motifs$`12h_vs_6h`$utr_5 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_6h_to_12h_5_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_6h_to_12h_5_utr')
            )
          )
)
```

```{r}
#| title: 12h to 24h 5' UTRs

ToD_genes_carrying_RBP_motifs$`24h_vs_12h`$utr_5 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_12h_to_24h_5_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_12h_to_24h_5_utr')
            )
          )
)
```

```{r}
#| title: 24h to 36h 5' UTRs

ToD_genes_carrying_RBP_motifs$`36h_vs_24h`$utr_5 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_24h_to_36h_5_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_24h_to_36h_5_utr')
            )
          )
)
```

```{r}
#| title: 36h to 48h 5' UTRs

ToD_genes_carrying_RBP_motifs$`48h_vs_36h`$utr_5 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_36h_to_48h_5_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_36h_to_48h_5_utr')
            )
          )
)
```

```{r}
#| title: 36h to 48h 3' UTRs

ToD_genes_carrying_RBP_motifs$`48h_vs_36h`$utr_3 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_36h_to_48h_3_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_36h_to_48h_3_utr')
            )
          )
)
```

```{r}
#| title: 48h to 72h 5' UTRs

ToD_genes_carrying_RBP_motifs$`72h_vs_48h`$utr_5 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_48h_to_72h_5_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_48h_to_72h_5_utr')
            )
          )
)
```

```{r}
#| title: 48h to 72h 3' UTRs

ToD_genes_carrying_RBP_motifs$`72h_vs_48h`$utr_3 %>%
  group_by(motif) %>%
  datatable(., 
          options = list(
            dom = 'Bfrtip',
            buttons = list(
              list(extend = 'csv', title = 'ToD_genes_RBP_motifs_48h_to_72h_3_utr'),
              list(extend = 'excel', title = 'ToD_genes_RBP_motifs_48h_to_72h_3_utr')
            )
          )
)
```

## Row {.tabset title="RNA Binding Proteins Background that are present in matched data (RNA and Protein)"}

::: {.card title="Nono / M307"}
The Nono protein is a multifunctional DNA- and RNA-binding protein involved in various nuclear processes:

-  **Binding**: Binds both double-stranded and single-stranded DNA/RNA.
-  **Splicing and Paraspeckles**: Involved in pre-mRNA splicing and the formation of nuclear paraspeckles, often in a complex with SFPQ.
-  **DNA Repair**: Plays a role in DNA repair processes like non-homologous end joining (NHEJ).
-  **Transcription Regulation**: Regulates transcription by interacting with promoters and enhancers.
-  **Circadian Rhythm**: Modulates the circadian clock by repressing CLOCK-BMAL1 activity.
-  **Synaptic Function**: Important for GABAergic synapse organization and synaptic RNA regulation.
-  **Neuronal Differentiation**: Influences neuronal differentiation through epigenetic modifications.
-  **Immune Response**: Participates in innate immune responses to DNA viruses.
:::

::: {.card title="Ncl / M245"}
Nucleolin is a predominant nucleolar protein in growing eukaryotic cells, primarily associated with intranucleolar chromatin and pre-ribosomal particles. Its key functions include:

-  **Chromatin Decondensation**: Induces chromatin decondensation by binding to histone H1.
-  **Ribosome Assembly**: Plays a role in pre-rRNA transcription and ribosome assembly.
-  **Transcriptional Elongation**: May be involved in the process of transcriptional elongation.
-  **RNA Binding**: Binds RNA oligonucleotides with 5'-UUAGGG-3' repeats more tightly than telomeric single-stranded DNA with 5'-TTAGGG-3' repeats.
:::

::: {.card title="Ybx2 / M082"}
This protein is a key component of messenger ribonucleoprotein particles (mRNPs) and plays a crucial role in the regulation of germ cell mRNA stability and translation. Its functions include:

-  **mRNA Binding**: Binds full-length mRNA with high affinity in a sequence-independent manner and binds short RNA sequences with low affinity.
-  **Y-box Interaction**: Binds to the Y-box consensus promoter element, potentially marking mRNAs for cytoplasmic storage.
-  **Cytoplasmic Retention**: Essential for the cytoplasmic retention of maternal mRNAs.
-  **mRNA Storage**: May link transcription and mRNA storage/translational delay by marking specific mRNAs for cytoplasmic storage.
:::

::: {.card title="Snrnp70 / M068"}
This gene encodes a subunit of the U1 snRNP (small nuclear ribonucleic particle), one of at least five snRNPs to comprise the spliceosome, which functions in processing of pre-mRNAs. The U1 snRNP has been shown to be important in defining the 5' splice site. Alternative splicing results in multiple transcript variants
:::

::: {.card title="Hnrnph2 / M151"}
This protein is a component of the heterogeneous nuclear ribonucleoprotein (hnRNP) complexes which provide the substrate for the processing events that pre-mRNAs undergo before becoming functional, translatable mRNAs in the cytoplasm.
:::

::: {.card title="Rbm46 / M052"}
Enables RNA binding activity. Involved in several processes, including cell cycle switching, mitotic to meiotic cell cycle; meiotic nuclear division; and spermatid differentiation. Acts upstream of or within mRNA stabilization and trophectodermal cell differentiation. Located in cytoplasm and nucleus. Is expressed in central nervous system; craniocervical region bone; dorsal root ganglion; lung; and rib. Orthologous to human RBM46
:::

::: {.card title="Srsf3 / M056"}
Splicing factor, which binds the consensus motif 5'-C[ACU][AU]C[ACU][AC]C-3' within pre-mRNA and promotes specific exons inclusion during alternative splicing. Interaction with YTHDC1, a RNA-binding protein that recognizes and binds N6-methyladenosine (m6A)-containing RNAs, promotes recruitment of SRSF3 to its mRNA-binding elements adjacent to m6A sites within exons. Also functions as an adapter involved in mRNA nuclear export. Binds mRNA which is thought to be transferred to the NXF1-NXT1 heterodimer for export (TAP/NXF1 pathway); enhances NXF1-NXT1 RNA-binding activity. Involved in nuclear export of m6A-containing mRNAs via interaction with YTHDC1: interaction with YTHDC1 facilitates m6A-containing mRNA-binding to both SRSF3 and NXF1, promoting mRNA nuclear export
:::

::: {.card title="Eif2s1 / M231"}
This protein is a member of the eIF2 complex, playing a crucial role in the early steps of protein synthesis and stress response. Key functions include:

-  **Protein Synthesis Initiation**: Forms a ternary complex with GTP and initiator tRNA, binding to the 40S ribosomal subunit to form the 43S pre-initiation complex, and later the 80S initiation complex after GTP hydrolysis.
-  **GTP/GDP Exchange**: Requires GDP-GTP exchange, catalyzed by eIF2B, for recycling and initiating another round of protein synthesis.
-  **Integrated Stress Response (ISR)**: Phosphorylation by stress-sensing kinases turns eIF2-alpha into an inhibitor of global protein synthesis, reducing cap-dependent translation while promoting translation of ISR-specific mRNAs like ATF4 and QRICH1.
-  **Mitophagy Activation**: Acts as an activator of mitophagy in response to mitochondrial damage by relocating to the mitochondrial surface upon phosphorylation, triggering PRKN-independent mitophagy.
:::

::: {.card title="Tardbp / M074"}
-  **DNA and RNA Binding**: Enables sequence-specific DNA binding at RNA polymerase II cis-regulatory regions and binds pre-mRNA introns.
-  **Regulatory Roles**: Involved in positively regulating protein import into the nucleus, circadian rhythm, and protein stability.
-  **RNA Splicing**: Acts upstream of or within RNA splicing processes.
-  **Localization and Expression**: Located in the nucleus and expressed in various structures, including the central nervous system, heart, genitourinary system, branchial arch, and early conceptus.
-  **Research and Disease Study**: Used in studies related to Grn-related frontotemporal lobar degeneration with TDP-43 inclusions, amyotrophic lateral sclerosis type 10, and frontotemporal dementia.
-  **Human Orthologs**: The human ortholog, TARDBP, is implicated in Parkinson's disease, amyotrophic lateral sclerosis, and motor neuron disease.
:::

::: {.card title="Rbm4b / M109"}
-  **RNA Binding**: Predicted to enable RNA binding activity.
-  **Circadian Regulation**: Involved in the circadian regulation of gene expression and the entrainment of the circadian clock by photoperiod.
-  **Translation Regulation**: Plays a role in the regulation of translation.
-  **Gene Expression**: Acts upstream of or within the positive regulation of gene expression.
-  **Cellular Localization**: Predicted to be located in the cytosol and nucleoplasm and active in nuclear specks.
-  **Complex Participation**: Predicted to be part of a protein-containing complex.
-  **Expression**: Expressed in various systems, including the alimentary, brain, cardiovascular, genitourinary, and hemolymphoid systems.
:::

::: {.card title="Raly / M150"}
Predicted to enable RNA binding activity and identical protein binding activity. Predicted to be located in nucleoplasm. Predicted to be active in nucleus. Is expressed in brain; cortical plate; spinal cord; and trigeminal ganglion
:::

::: {.card title="Tial1 / M075"}
-  **Alternative Splicing**: Involved in alternative pre-mRNA splicing, particularly activating splicing of exons with weak 5' splice sites followed by a U-rich stretch, including its own pre-mRNA and TIA1 mRNA.
-  **RNA Preference**: Shows a preference for binding uridine-rich RNAs.
-  **Exon Inclusion**: Promotes the inclusion of exon 5 in TIA1, leading to the production of the long isoform (isoform a) of TIA1.
-  **Stress Granules Formation**: Plays a role in forming cytoplasmic stress granules, acting downstream of stress-induced EIF2S1/EIF2A phosphorylation to recruit untranslated mRNAs to these granules.
:::

::: {.card title="Cpeb4 / M149"}
-   **RNA Binding**: Binds to the cytoplasmic polyadenylation element (CPE), a uridine-rich sequence in the mRNA 3'-UTR, causing a conformational change similar to the Venus fly trap mechanism.
-   **ER Stress Response**: Regulates the activation of the unfolded protein response (UPR) during ER stress in the liver by maintaining the translation of CPE-regulated mRNAs when global protein synthesis is inhibited.
-   **Cell Cycle and Cytokinesis**: Essential for cell cycle progression, particularly cytokinesis and chromosomal segregation.
-   **Oncogenic Role**: Acts as an oncogene by promoting tumor growth and progression through the positive regulation of t-plasminogen activator/PLAT translation.
-   **Melanocyte Proliferation**: Stimulates the proliferation of melanocytes.
-   **Synaptic Plasticity**: Unlike CPEB1 and CPEB3, it does not play a role in synaptic plasticity, learning, and memory.
:::

::: {.card title="Rbmx / M344"}
-  **Gene Transcription and Tumor Suppression**: Binds to and stimulates transcription from the TXNIP gene promoter, potentially playing a role in tumor suppression. When associated with SAFB, it also stimulates transcription from the SREBF1 promoter.
-  **mRNA Association**: Associates with nascent mRNAs transcribed by RNA polymerase II and is part of the supraspliceosome complex, influencing pre-mRNA alternative splice site selection.
-  **Alternative Splicing**: Can activate or suppress exon inclusion; works with TRA2B to promote exon 7 inclusion in SMN2 and represses the splicing of MAPT/Tau exon 10.
-  **RNA Binding**: Binds preferentially to single-stranded 5'-CC[A/C]-rich RNA sequence motifs and likely binds RNA as a homodimer. It also binds non-specifically to pre-mRNAs.
-  **Cytoplasmic Role**: Involved in cytoplasmic TNFR1 trafficking, promoting IL-1-beta-mediated cleavage of TNFR1 ectodomains and the release of TNFR1 in exosome-like vesicles.
:::

::: {.card title="Pcbp2 / M043"}
-   **Binding Specificity**: Prefers binding to oligo dC and also binds poly(rC) and poly(rU).
-   **Antiviral Signaling Regulation**: Acts as a negative regulator of antiviral signaling, specifically by:
   -  **MAVS Signaling**: Serving as an adapter between MAVS and the E3 ubiquitin ligase ITCH, facilitating MAVS ubiquitination and degradation.
   -  **cGAS-STING Pathway**: Interacting with CGAS to prevent the formation of liquid-like droplets necessary for CGAS activation.
-   **Erythropoiesis**: Along with PCBP1, it is required for erythropoiesis, possibly through the regulation of mRNA splicing.
:::

::: {.card title="Prrc1 / M044"}
Predicted to enable protein kinase A regulatory subunit binding activity. Predicted to be involved in epithelial structure maintenance. Predicted to be located in Golgi apparatus. Predicted to be active in cytoplasm. Is expressed in Meckel's cartilage; chondrocranium; and olfactory epithelium.
:::

## Row
```{r}
#| title: RBP RNA and Protein expression over time points 
library(ggrepel)
library(patchwork)
trans_prot_data <- here("data/processed/yang_paper/", "trans_prot_data.rds") %>% readRDS()
RNA_binding_proteins <- here("data/processed/", "RNA_binding_proteins.rds") %>% readRDS()

long_trans_data <- trans_prot_data %>%
  filter(mgi_symbol %in% RNA_binding_proteins) %>%
  dplyr::select(mgi_symbol, h0_trans, h1_trans, h6_trans, h12_trans, h24_trans, h36_trans, h48_trans, h72_trans) %>%
  pivot_longer(
    cols = starts_with("h"), # Select all columns starting with 'h'
    names_to = "time_point", # New column for the names of the columns
    values_to = "trans_value" # New column for the values
  ) %>%
  mutate(time_point = factor(time_point, levels = c("h0_trans", "h1_trans", "h6_trans", "h12_trans", "h24_trans", "h36_trans", "h48_trans", "h72_trans")),
         trans_value = log10(trans_value))

long_prot_data <- trans_prot_data %>%
  filter(mgi_symbol %in% RNA_binding_proteins) %>%
  dplyr::select(mgi_symbol, h0_prot, h1_prot, h6_prot, h12_prot, h24_prot, h36_prot, h48_prot, h72_prot) %>%
  pivot_longer(
    cols = starts_with("h"), # Select all columns starting with 'h'
    names_to = "time_point", # New column for the names of the columns
    values_to = "prot_value" # New column for the values
  ) %>%
  mutate(time_point = factor(time_point, levels = c("h0_prot", "h1_prot", "h6_prot", "h12_prot", "h24_prot", "h36_prot", "h48_prot", "h72_prot")))

p1 <- ggplot(long_trans_data, aes(x = time_point, y = trans_value, group = mgi_symbol, color = mgi_symbol)) +
  geom_line() + # Add lines for each mgi_symbol
  geom_point() + # Add points at each time point
  labs(
    title = "RNA",
    x = "Time Point",
    y = "log10(Gene Expression Value)"
  ) +
  theme_minimal() + # Use a clean theme
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"
  ) +
  geom_text_repel(
    data = long_trans_data %>%
      group_by(mgi_symbol) %>%
      filter(time_point == "h72_trans"),
    aes(label = mgi_symbol),
    size = 3,
    nudge_x = 1.5,
    direction = "y",
    hjust = 0
  )

p2 <- ggplot(long_prot_data, aes(x = time_point, y = prot_value, group = mgi_symbol, color = mgi_symbol)) +
  geom_line() +
  geom_point() +
  labs(
    title = "Protein",
    x = "Time Point",
    y = "Protein Expression Value"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"
  ) +
  geom_text_repel(
    data = long_prot_data %>%
      group_by(mgi_symbol) %>%
      filter(time_point == "h72_prot"),
    aes(label = mgi_symbol),
    size = 3,
    nudge_x = 1.5,
    direction = "y",
    hjust = 0
  ) 

p1 + p2 
```

# UTR Metrics

## Row 

```{r}
#| include: false
background_utrs <- here("data/reference", "background_utrs.rds") %>% readRDS()
ToD_utrs_list <- here("data/reference", "ToDC_all_utrs.rds") %>% readRDS() 

ToD_utr_5_list <- lapply(ToD_utrs_list, function(x) x$utr_5)
ToD_utr_5 <- do.call(rbind, ToD_utr_5_list)

ToD_utr_3_list <- lapply(ToD_utrs_list, function(x) x$utr_3)
ToD_utr_3 <- do.call(rbind, ToD_utr_3_list)
```


```{r}
#| title: Nucleotide Content in 5' UTRs of ToD candidates vs all 5' UTRs
# Reshape the data for plotting and set factor levels for the first dataset
long_data1 <- ToD_utr_5 %>%
  select(A_count_percentage, C_count_percentage, G_count_percentage, T_count_percentage, GC_content_percentage) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "value") %>%
  mutate(variable = factor(variable, levels = c("A_count_percentage", "C_count_percentage", "G_count_percentage", "T_count_percentage", "GC_content_percentage")))

# Reshape the data for plotting and set factor levels for the second dataset
long_data2 <- background_utrs$utr_5 %>%
  select(A_count_percentage, C_count_percentage, G_count_percentage, T_count_percentage, GC_content_percentage) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "value") %>%
  mutate(variable = factor(variable, levels = c("A_count_percentage", "C_count_percentage", "G_count_percentage", "T_count_percentage", "GC_content_percentage")))

# Create the overlayed boxplot
ggplot() +
  # First boxplot with transparency
  geom_boxplot(data = long_data1, aes(x = variable, y = value, fill = variable)) +
  # Second boxplot on top
  geom_boxplot(data = long_data2, aes(x = variable, y = value, color= "red"), alpha = 0.03) +
  theme_minimal() +
  stat_summary(data = long_data1, fun = mean, geom = "text", aes(x = variable, y = value, label = sprintf("%.2f", ..y..)), vjust = -0.5, color = "black") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right") +
  labs(y = "Percentage", x = "",fill = "Nucleotide Content", color = "all 5' UTRs") +
  ggsci::scale_color_nejm() 
```

```{r}
#| title: Nucleotide Content in 3' UTRs of ToD candidates vs all 3' UTRs
# Reshape the data for plotting and set factor levels for the first dataset
long_data1 <- ToD_utr_3 %>%
  select(A_count_percentage, C_count_percentage, G_count_percentage, T_count_percentage, GC_content_percentage) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "value") %>%
  mutate(variable = factor(variable, levels = c("A_count_percentage", "C_count_percentage", "G_count_percentage", "T_count_percentage", "GC_content_percentage")))

# Reshape the data for plotting and set factor levels for the second dataset
long_data2 <- background_utrs$utr_3 %>%
  select(A_count_percentage, C_count_percentage, G_count_percentage, T_count_percentage, GC_content_percentage) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "value") %>%
  mutate(variable = factor(variable, levels = c("A_count_percentage", "C_count_percentage", "G_count_percentage", "T_count_percentage", "GC_content_percentage")))

# Create the overlayed boxplot
ggplot() +
  # First boxplot with transparency
  geom_boxplot(data = long_data1, aes(x = variable, y = value, fill = variable)) +
  # Second boxplot on top
  geom_boxplot(data = long_data2, aes(x = variable, y = value, color= "red"), alpha = 0.03) +
  theme_minimal() +
  stat_summary(data = long_data1, fun = mean, geom = "text", aes(x = variable, y = value, label = sprintf("%.2f", ..y..)), vjust = -0.5, color = "black") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right") +
  labs(y = "Percentage", x = "",fill = "Nucleotide Content", color = "all 3' UTRs") +
  ggsci::scale_color_nejm() 
```
