...
ToD_candidates_list <- here::here("data/processed", "ToD_candidates_list.rds") |> readRDS()
```
  ...
  for (comp in comparison_names) {
    ...
    for (ont in ontologies) {
      ego <- enrichGO(gene         = cand_entrez$ENTREZID,
                      ...
                      ont          = ont,
                      pAdjustMethod = "BH",
                      pvalueCutoff = 1,    # lenient thresholds for exploration
                      qvalueCutoff = 1,
                      readable     = TRUE)
